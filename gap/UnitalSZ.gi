...
function( bmat )
	...
	for i in [1..q^3] do
		for j in [i+1..q^3+1] do
			if SizeBlist(IntersectionBlist(bmattr[i],bmattr[j]))<>1 then
				return false;
			fi;
		od;
	od;
...
function( bmat )
	local q,i,j;
	q:=SizeBlist(bmat[1])-1;
	if Length(bmat)<>q^2*(q^2-q+1) or ForAny(bmat,x->Length(x)<>q^3+1) then
...
function( bmat )
	...
	q:=SizeBlist(bmat[1])-1;
	uni:=Objectify(
		NewType(AU_UnitalDesignFamily,IsAU_UnitalDesign and IsAU_UnitalDesignRep),
		rec(bmat:=Set(bmat))
	);
	SetOrder(uni,q);
	return uni;
...
function( q )
	...
	bls:=Union([1,2],First(Orbits(Stabilizer(pgl,[1,2],OnTuples)),x->Length(x)=q-1));
...
#############################################################################
##  BLOCK, POINTS, INCIDENT DIGRAPHS
##  -------------------------------------------------------------------------
...
function( u )
	...
	return Digraph(
		[1..q^3+1+q^2*(q^2-q+1)],
		function(x,y) return x<=q^3+1 and y>q^3+1 and u!.bmat[y-q^3-1][x]; end
	);
end );
...
InstallOtherMethod( \^, "for an abstract unitals and a permutation",
	...
	function( u, perm )
		return AU_UnitalByBlistListNC(List(u!.bmat,x->Permuted(x,perm)));
end );
...
InstallMethod( AU_Isomorphism, "for two abstract unitals",
	...
	function( u1, u2 )
		...
		ret:=IsomorphismDigraphs(AU_IncidenceDigraph(u1),AU_IncidenceDigraph(u2));
		if ret=fail then
			return fail;
		else
