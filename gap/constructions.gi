...
function( n )
    ...
    # For parameters n<=20, the only known cyclic unitals are for n=4 and n=6,
    # and they are unique.
    # In order to force computation for an arbitrary n, comment out the next
    # lines:
    ...
    gq := Z(q)^( ( q - 1 )/( p - 1 ) );
    gp := Z(p)^2;
    t := First( Reversed( DivisorsInt( p - 1 ) ),
                x -> Gcd( x, ( q - 1 )/( p - 1 ) ) = 1 );
    gamma := Z(q)^t;
